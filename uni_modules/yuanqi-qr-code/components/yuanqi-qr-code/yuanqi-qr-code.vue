...
<script>
	const canvasId=`yuanqi_${Math.round(Math.random()*10000).toString(16)}`;
	...
	export default {
		...
		// #ifdef APP-VUE || MP-ALIPAY || MP-TOUTIAO || H5
		beforeMount(){
		...
		methods: {
			...
			drawLogo(src) {
				...
				_ctx.drawImage(src, logoX, logoY, logoSize, logoSize)
			},
			draw(oQRCode) {
				const _ctx = this._ctx;
				...
				this.clear();
				if (this.bgImg) {
					_ctx.drawImage(this.bgImg, 0, 0, this.canvasSize, this.canvasSize)
