  ...
  <view class="pages_home_tab">
    ...
    <view class="pages_home_container">
      <swiper
        ...
        indicator-active-color="rgba(229, 25, 55, .1)"
      >
      ...
      <view class="home_icon">
        ...
        <view class="icon_list" v-else>
          ...
          <view
            class="icon_item_two"
            v-for="(item, index) in loginType == 1 ? iconBar1 : iconBar3"
            :key="index"
            @click="toPage(item.path,item.flag,item.tabIndex)"
          >
            <image :src="item.img" mode="widthFix" />
      ...
      <view class="home_firm_query">
        ...
        <view class="firm_query_container">
          <view class="firm_query_item" v-for="item in queryList" :key="item.id">
      ...
      warnList: [
        {
          ...
          //提醒内容
          content:
            "【广知科技有限公司】消防器材未按照标准规范摆放消防通道有障碍物存放",
          overdue: "4天", //逾期天数
